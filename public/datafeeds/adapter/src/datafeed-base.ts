...
export interface SearchSymbolsResponse extends Array<SearchSymbolResultItem> {
	s?: undefined;
}
...
type DatafeedMarkType<T extends TimescaleMark | Mark> = {
	[K in keyof T]: T[K] | T[K][];
} & {
	id: (string | number)[];
};

type DatafeedMark = DatafeedMarkType<Mark>;
type DatafeedTimescaleMark = DatafeedMarkType<TimescaleMark>;

// function extractField<Field extends keyof Mark>(data: DatafeedMark, field: Field, arrayIndex: number): Mark[Field];
// function extractField<Field extends keyof TimescaleMark>(data: DatafeedTimescaleMark, field: Field, arrayIndex: number): TimescaleMark[Field];
function extractField<Field extends keyof (TimescaleMark & Mark)>(data: DatafeedMark & DatafeedTimescaleMark, field: Field, arrayIndex: number): (TimescaleMark & Mark)[Field] {
	const value = data[field];
	return Array.isArray(value) ? value[arrayIndex] : value;
}

export class DatafeedBase implements IExternalDatafeed, IDatafeedQuotesApi, IDatafeedChartApi {
	...
	protected constructor(socket: any, datafeedURL: string, quotesProvider: IQuotesProvider, requester: Requester, updateFrequency: number = 10 * 1000) {
		this._socket = socket;
		...
		this._dataPulseProvider = new DataPulseProvider(this._socket, updateFrequency);
		this._quotesPulseProvider = new QuotesPulseProvider(this._quotesProvider);

		this._configurationReadyPromise = this._requestConfiguration()
			.then((configuration: Configuration | null) => {
	...
	public getMarks(symbolInfo: LibrarySymbolInfo, startDate: number, endDate: number, onDataCallback: GetMarksCallback<Mark>, resolution: ResolutionString): void {
		...
		this._send('marks', requestParams)
			...
			.catch((error?: string | Error) => {
				logMessage(`Datafeed: Request marks failed: ${getErrorMessage(error)}`);
				onDataCallback([]);
			});
	}
	...
	public getTimescaleMarks(symbolInfo: LibrarySymbolInfo, startDate: number, endDate: number, onDataCallback: GetMarksCallback<TimescaleMark>, resolution: ResolutionString): void {
		...
		this._send('timescale_marks', requestParams)
			...
			.catch((error?: string | Error) => {
				logMessage(`Datafeed: Request timescale marks failed: ${getErrorMessage(error)}`);
				onDataCallback([]);
			});
	}
	...
	public getServerTime(callback: ServerTimeCallback): void {
		...
		this._send('time')
			...
			.catch((error?: string | Error) => {
				logMessage(`Datafeed: Fail to load server time, error=${getErrorMessage(error)}`);
			});
	}
			...
			this._send('search', params)
				.then((response: any) => {
					if (response.s !== undefined) {
						logMessage(`Datafeed: search symbols error=${response.errmsg}`);
						onResult([]);
						return;
					}
				...
				.catch((reason?: string | Error) => {
					logMessage(`Datafeed: Search symbols for '${userInput}' failed. Error=${getErrorMessage(reason)}`);
					onResult([]);
				});
		} else {
	...
	public resolveSymbol(symbolName: string, onResolve: ResolveCallback, onError: ErrorCallback): void {
		...
		if (!this._configuration.supports_group_request) {
			...
			this._send('symbols', params)
				...
				.catch((reason?: string | Error) => {
					logMessage(`Datafeed: Error resolving symbol: ${getErrorMessage(reason)}`);
					onError('unknown_symbol');
				});
		} else {
			if (this._symbolsStorage === null) {
	...
	protected _requestConfiguration(): Promise<Configuration | null> {
		return this._send<Configuration>('config')
			.catch((reason?: string | Error) => {
				logMessage(`Datafeed: Cannot get datafeed configuration - use default, error=${getErrorMessage(reason)}`);
				return null;
			});
	}
	...
	private _setupWithConfiguration(configurationData: Configuration): void {
		...
		logMessage(`Datafeed: Initialized with ${JSON.stringify(configurationData)}`);
	}
}
